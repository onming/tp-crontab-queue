...
use think\Log;
use app\queue\controller\Producer;

class Crontab extends Command
{

    const WRITE_LOG = true;//是否写入日志

    protected function configure()
    {
        $this->setName('crontab')->setDescription('每日定时脚本生成器');
    }
    ...
    public static function addTask($class, $param = array(), $cron_time)
    {
        if(self::check(time(), $cron_time)){
            $result = Producer::addQueue($class, $param);
            if(self::WRITE_LOG){
                Log::init([
                    'type'  =>  'File',
